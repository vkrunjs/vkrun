...
describe("Validator NotEqual Method Types", () => {
  it("should infer a buffer", () => {
    const bufferSchema = schema().buffer().notEqual(Buffer.from("hello"));
  ...
  it("should infer a buffer or null", () => {
    const bufferSchema = schema().buffer().notEqual(Buffer.from("hello")).nullable();
    const value = bufferSchema.test(Buffer.from("hello")).value;
  ...
  it("should infer a buffer, null, or undefined", () => {
    const bufferSchema = schema().buffer().notEqual(Buffer.from("hello")).nullable().notRequired();
    const value = bufferSchema.test(Buffer.from("hello")).value;
