...
describe("Validator Max Method Types", () => {
  it("should infer a Date", () => {
    const dateSchema = schema().date().max({ max: new Date() });
  ...
  it("should infer a Date or null", () => {
    const dateSchema = schema().date().max({ max: new Date() }).nullable();
    const value = dateSchema.test(new Date()).value;
  ...
  it("should infer a Date null, or undefined", () => {
    const dateSchema = schema().date().max({ max: new Date() }).nullable().notRequired();
    const value = dateSchema.test(new Date()).value;
