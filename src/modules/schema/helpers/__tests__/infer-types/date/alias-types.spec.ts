...
describe("Validator Alias Method Types", () => {
  it("should infer a Date", () => {
    const dateSchema = schema().date().alias("value name");
  ...
  it("should infer a Date or null", () => {
    const dateSchema = schema().date().alias("value name").nullable();
    const value = dateSchema.test(new Date()).value;
  ...
  it("should infer a Date, null, or undefined", () => {
    const dateSchema = schema().date().alias("value name").nullable().notRequired();
    const value = dateSchema.test(new Date()).value;
