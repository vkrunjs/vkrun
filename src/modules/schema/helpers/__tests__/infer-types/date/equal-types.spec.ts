...
describe("Validator Equal Method Types", () => {
  it("should infer a date", () => {
    const dateSchema = schema().date().equal(new Date());
  ...
  it("should infer a date or null", () => {
    const dateSchema = schema().date().equal(new Date()).nullable();
    const value = dateSchema.test(new Date()).value;
  ...
  it("should infer a date, null, or undefined", () => {
    const dateSchema = schema().date().equal(new Date()).nullable().notRequired();
    const value = dateSchema.test(new Date()).value;
