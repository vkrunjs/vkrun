...
describe("Validator Min Method Types", () => {
  it("should infer a bigInt", () => {
    const bigIntSchema = schema().bigInt().min({ min: 1n });
  ...
  it("should infer a bigInt or null", () => {
    const bigIntSchema = schema().bigInt().min({ min: 1n }).nullable();
    const value = bigIntSchema.test(123n).value;
  ...
  it("should infer a bigInt, null, or undefined", () => {
    const bigIntSchema = schema().bigInt().min({ min: 1n }).nullable().notRequired();
    const value = bigIntSchema.test(123n).value;
