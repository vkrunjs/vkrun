...
describe("Validator MinWord Method Types", () => {
  it("should infer a string", () => {
    const stringSchema = schema().string().minWord({ min: 1 });
  ...
  it("should infer a string or null", () => {
    const stringSchema = schema().string().minWord({ min: 1 }).nullable();
    const value = stringSchema.test("").value;
  ...
  it("should infer a string, null, or undefined", () => {
    const stringSchema = schema().string().minWord({ min: 1 }).nullable().notRequired();
    const value = stringSchema.test("").value;
