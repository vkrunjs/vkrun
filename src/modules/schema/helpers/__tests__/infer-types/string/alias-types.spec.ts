...
describe("Validator Alias Method Types", () => {
  it("should infer a string", () => {
    const stringSchema = schema().string().alias("value name");
  ...
  it("should infer a string or null", () => {
    const stringSchema = schema().string().alias("value name").nullable();
    const value = stringSchema.test("").value;
  ...
  it("should infer a string, null, or undefined", () => {
    const stringSchema = schema().string().alias("value name").nullable().notRequired();
    const value = stringSchema.test("").value;
