...
describe("Validator Time Method Types", () => {
  it("should infer a string", () => {
    const stringSchema = schema().string().time({ type: "HH:MM" });
  ...
  it("should infer a string or null", () => {
    const stringSchema = schema().string().time({ type: "HH:MM" }).nullable();
    const value = stringSchema.test("").value;
  ...
  it("should infer a string, null, or undefined", () => {
    const stringSchema = schema().string().time({ type: "HH:MM" }).nullable().notRequired();
    const value = stringSchema.test("").value;
