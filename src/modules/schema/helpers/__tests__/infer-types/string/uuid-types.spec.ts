...
describe("Validator UUID Method Types", () => {
  it("should infer a string", () => {
    const stringSchema = schema().string().UUID();
  ...
  it("should infer a string or null", () => {
    const stringSchema = schema().string().UUID().nullable();
    const value = stringSchema.test("").value;
  ...
  it("should infer a string, null, or undefined", () => {
    const stringSchema = schema().string().UUID().nullable().notRequired();
    const value = stringSchema.test("").value;
