...
describe("Validator Alias Method Types", () => {
  it("should infer a number", () => {
    const numberSchema = schema().number().alias("value name");
  ...
  it("should infer a number or null", () => {
    const numberSchema = schema().number().alias("value name").nullable();
    const value = numberSchema.test(123).value;
  ...
  it("should infer a number, null, or undefined", () => {
    const numberSchema = schema().number().alias("value name").nullable().notRequired();
    const value = numberSchema.test(123).value;
