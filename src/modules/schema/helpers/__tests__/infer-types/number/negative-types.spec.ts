...
describe("Validator Negative Method Types", () => {
  it("should infer a number", () => {
    const numberSchema = schema().number().negative();
  ...
  it("should infer a number or null", () => {
    const numberSchema = schema().number().negative().nullable();
    const value = numberSchema.test(123).value;
  ...
  it("should infer a number, null, or undefined", () => {
    const numberSchema = schema().number().negative().nullable().notRequired();
    const value = numberSchema.test(123).value;
