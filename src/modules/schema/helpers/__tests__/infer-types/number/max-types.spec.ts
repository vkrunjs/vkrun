...
describe("Validator Max Method Types", () => {
  it("should infer a number", () => {
    const numberSchema = schema().number().max({ max: 1 });
  ...
  it("should infer a number or null", () => {
    const numberSchema = schema().number().max({ max: 1 }).nullable();
    const value = numberSchema.test(123).value;
  ...
  it("should infer a number, null, or undefined", () => {
    const numberSchema = schema().number().max({ max: 1 }).nullable().notRequired();
    const value = numberSchema.test(123).value;
