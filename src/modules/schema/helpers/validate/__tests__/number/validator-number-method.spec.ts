...
describe("Validator Number Method", () => {
  ...
  it("Should be able to validate the number method and return true if the value is of type number", () => {
    ...
    const sut = schema().number().validate(value);
  ...
  it("Should be able to validate the number method when value is promise and return true if the value is of type number", async () => {
    ...
    const sut = await schema().number().validateAsync(value());
  ...
  it("Should be able to validate the number method and passedAll to equal true if the value is of type number", () => {
    ...
    const sut = schema().number().test(value, "value_name");
  ...
  it("Should allow custom error message", () => {
    ...
    const sut = schema().number({ message: "[valueName] [value]!" }).test(value, "value_name");
  ...
  it("Should be able to validate the number and passAll method as equal to true when it is not required, undefined value and not of type number", () => {
    ...
    const sut = schema().number().notRequired().test(value, "value_name");
  ...
  it("Should be able to validate the number method and passedAll to equal true if the value is promise of type number", async () => {
    ...
    const sut = await schema().number().testAsync(value(), "value_name");
  ...
  it("Should be able to validate the number method and throw AnyError if the value is not of type number", () => {
    ...
    const sut = (): void => schema().number().throw(value, "value_name", AnyError);
  ...
  it("Should be able to validate the number method and throw Error if the value is a promise and is not of type number", async () => {
    ...
    const sut = async (): Promise<void> => await schema().number().throwAsync(value(), "value_name");

    await expect(sut).rejects.toThrow("value_name must be a number type!");
  });

  it("ensures that the typing is number", () => {
    const numberSchema = schema().number().min({ min: 3 });

    type NumberSchema = InferIn<typeof numberSchema>;

    const a: NumberSchema = 123;
  });

  it("ensures that the typing is number | null", () => {
    const numberSchema = schema().number().min({ min: 3 }).nullable();

    type NumberSchema = InferIn<typeof numberSchema>;

    const a: NumberSchema = 123;
    const b: NumberSchema = null;
  });

  it("ensures that the typing is number | null | undefined", () => {
    const numberSchema = schema().number().min({ min: 3 }).notRequired().nullable();

    type NumberSchema = InferIn<typeof numberSchema>;

    const a: NumberSchema = 123;
    const b: NumberSchema = null;
    const c: NumberSchema = undefined;
  });

  it("ensures that the typing InferIn and InferOut is of the correct type with parseTo method", () => {
    const number = schema().number();

    // Number To String
    const string = number.parseTo().string();

    type InputString = InferIn<typeof string>;
    type OutputString = InferOut<typeof string>;

    const inputString: InputString = 123;
    const outputString: OutputString = "string";

    // Number To BigInt
    const bigInt = number.parseTo().bigInt();

    type InputBigInt = InferIn<typeof bigInt>;
    type OutputBigInt = InferOut<typeof bigInt>;

    const inputBigInt: InputBigInt = 123;
    const outputBigInt: OutputBigInt = 123n;

    // Number To Boolean
    const boolean = number.parseTo().boolean();

    type InputBoolean = InferIn<typeof boolean>;
    type OutputBoolean = InferOut<typeof boolean>;

    const inputBoolean: InputBoolean = 123;
    const outputBoolean: OutputBoolean = false;

    // Number To Date
    const date = number.parseTo().date();

    type InputDate = InferIn<typeof date>;
    type OutputDate = InferOut<typeof date>;

    const inputDate: InputDate = 123;
    const outputDate: OutputDate = new Date();
  });
});
