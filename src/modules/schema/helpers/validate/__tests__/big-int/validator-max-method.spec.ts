...
describe("Validator Max BigInt Method", () => {
  it("Should not modify base schema when creating derived schema with max", () => {
    // Base schema
    const baseBigIntSchema = schema().bigInt().max({ max: 5n });
    ...
    // Valid value to test
    const validBigInt = 3n; // less than max

    // --- Base schema should only validate bigint less than or equal to max and not allow null/undefined ---
    ...
    expect(baseBigIntSchema.validate(6n)).toBeFalsy(); // above max
    ...
    expect(derivedNullableSchema.validate(6n)).toBeFalsy(); // above max
    ...
    expect(derivedNotRequiredNullableSchema.validate(6n)).toBeFalsy(); // above max
  ...
  it("Should be able to validate the max method and return true if the value is less than or equal to the reference", () => {
    expect(schema().bigInt().max({ max: 5n }).validate(5n)).toBeTruthy();
  ...
  it("Should be able to validate the max method and return false if list is invalid", () => {
    ...
    const sut = schema().bigInt().max({ max: 5n });
  ...
  it("Should be able to validate the max method when value is promise and return true if the value is less than or equal to the reference", async () => {
    ...
    const sut = await schema().bigInt().max({ max: 5n }).validateAsync(value());
  ...
  it("Should be able to validate the max method and passedAll to equal true if the value is value less than or equal to the reference", () => {
    ...
    const sut = schema().bigInt().max({ max: 5n }).test(value, "value_name");
  ...
  it("Should allow custom error message", () => {
    ...
    const sut = schema().bigInt().max({ max: 5n, message: "[valueName] [value]!" }).test(value, "value_name");
  ...
  it("Should be able to validate the max and passAll method as equal to true when it is not required", () => {
    ...
    const sut = schema().bigInt().max({ max: 5n }).notRequired().test(value, "value_name");
  ...
  it("Should be able to validate the max and passAll method as equal to true when it is nullable", () => {
    ...
    const sut = schema().bigInt().max({ max: 5n }).nullable().test(value, "value_name");
  ...
  it("Should be able to validate the max method and passedAll to equal true if the value is promise with a smaller bigint than the reference", async () => {
    ...
    const sut = await schema().bigInt().max({ max: 5n }).testAsync(value(), "value_name");
  ...
  it("Should be able to validate the max method and throw AnyError if the value is not bigint", () => {
    ...
    const sut = (): void => schema().bigInt().max({ max: 5n }).throw(value, "value_name", AnyError);
  ...
  it("Should be able to validate the max method and throw Error if the value is a promise and is not bigint", async () => {
    ...
    const sut = async (): Promise<void> => await schema().bigInt().max({ max: 5n }).throwAsync(value(), "value_name");
