import { SchemaMethod, SchemaSuccessTest, SchemaTests, SchemaErrorTest } from "../../../index";
...
export const runValidatorBlockAsync = async (
  params: {
    currentValue: any;
    valueName: string;
  ...
  const validateOtherMethods = async (rule: any): Promise<void> => {
    ...
    if (rule.method === "object") {
      ...
      await validateCustom({ ...validateMethodParams, ...rule });
