...
describe('HTTP status handlers', () => {
  it('Should return status code and injected content', async () => {
    ...
    expect(httpStatus.nonAuthoritativeInformation('NON_AUTHORITATIVE_INFORMATION')).toEqual({ statusCode: 203, content: 'NON_AUTHORITATIVE_INFORMATION' })
    ...
    expect(httpStatus.proxyAuthenticationRequired('PROXY_AUTHENTICATION_REQUIRED')).toEqual({ statusCode: 407, content: 'PROXY_AUTHENTICATION_REQUIRED' })
    ...
    expect(httpStatus.unsupportedMediaType('UNSUPPORTED_MEDIA_TYPE')).toEqual({ statusCode: 415, content: 'UNSUPPORTED_MEDIA_TYPE' })
    expect(httpStatus.requestedRangeNotSatisfiable('REQUESTED_RANGE_NOT_SATISFIABLE')).toEqual({ statusCode: 416, content: 'REQUESTED_RANGE_NOT_SATISFIABLE' })
    ...
    expect(httpStatus.insufficientSpaceOnResource('INSUFFICIENT_SPACE_ON_RESOURCE')).toEqual({ statusCode: 419, content: 'INSUFFICIENT_SPACE_ON_RESOURCE' })
    ...
    expect(httpStatus.unprocessableEntity('UNPROCESSABLE_ENTITY')).toEqual({ statusCode: 422, content: 'UNPROCESSABLE_ENTITY' })
    ...
    expect(httpStatus.preconditionRequired('PRECONDITION_REQUIRED')).toEqual({ statusCode: 428, content: 'PRECONDITION_REQUIRED' })
    expect(httpStatus.tooManyRequests('TOO_MANY_REQUESTS')).toEqual({ statusCode: 429, content: 'TOO_MANY_REQUESTS' })
    expect(httpStatus.requestHeaderFieldsTooLarge('REQUEST_HEADER_FIELDS_TOO_LARGE')).toEqual({ statusCode: 431, content: 'REQUEST_HEADER_FIELDS_TOO_LARGE' })
    expect(httpStatus.unavailableForLegalReasons('UNAVAILABLE_FOR_LEGAL_REASONS')).toEqual({ statusCode: 451, content: 'UNAVAILABLE_FOR_LEGAL_REASONS' })
    expect(httpStatus.internalServerError('INTERNAL_SERVER_ERROR')).toEqual({ statusCode: 500, content: 'INTERNAL_SERVER_ERROR' })
    ...
    expect(httpStatus.httpVersionNotSupported('HTTP_VERSION_NOT_SUPPORTED')).toEqual({ statusCode: 505, content: 'HTTP_VERSION_NOT_SUPPORTED' })
    expect(httpStatus.insufficientStorage('INSUFFICIENT_STORAGE')).toEqual({ statusCode: 507, content: 'INSUFFICIENT_STORAGE' })
    expect(httpStatus.networkAuthenticationRequired('NETWORK_AUTHENTICATION_REQUIRED')).toEqual({ statusCode: 511, content: 'NETWORK_AUTHENTICATION_REQUIRED' })
  })
})
