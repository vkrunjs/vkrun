...
import { createHttpRequest, createHttpResponse, customResponse, formatResponseData } from "./helpers";
import { SuperRequest, SuperRequestError, SuperRequestSuccess, VkrunApp } from "../types";
...
export const superRequest = (app: VkrunApp): SuperRequest => {
  ...
  const request = async (method: string, path: any, data: any, options?: Record<string, any>): Promise<SuperRequestSuccess> => {
    ...
    return await new Promise(async (resolve, reject) => {
      ...
      const serverResponse = await _app._reqWithoutServer(httpRequest, customResponse(httpResponse));
